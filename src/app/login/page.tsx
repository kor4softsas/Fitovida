...
import { Leaf, Mail, Lock, User, Eye, EyeOff, ArrowLeft, AlertCircle } from 'lucide-react';
import { cn } from '@/lib/utils';
import gsap from 'gsap';

// Error message component with GSAP animation - moved outside to prevent re-creation
const ErrorMessage = memo(({ message }: { message?: string }) => {
  ...
  // Always render the container, but it starts hidden
  return (
    <div 
      ref={containerRef} 
    ...
    >
      <div 
        ref={contentRef}
        className="flex items-center gap-1.5 text-red-500 text-xs pt-1.5 pb-0.5"
...
export default function LoginPage() {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    // Solo actualizar si el error cambió
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-black/5 border border-[var(--border)] p-6 sm:p-8 max-h-[calc(100vh-120px)] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-3" noValidate>
            ...
            <button
              type="submit"
              className="w-full py-3 bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white font-medium rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-[var(--primary)]/25 mt-4"
            >
          ...
          {mode === 'login' && (
            <>
              <div className="flex items-center gap-4 my-5">
                ...
                <span className="text-xs text-[var(--muted)]">o continúa con</span>
                <div className="flex-1 h-px bg-[var(--border)]" />
              </div>

              {/* Social login */}
              <div className="flex gap-3">
                <button className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-[var(--border)] rounded-xl hover:bg-[var(--background)] transition-colors">
